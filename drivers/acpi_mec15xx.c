...
void acpi_set_flag(enum acpi_ec_interface num, u8_t type, bool val)
{
	ACPI_EC_Type *acpi_ec_regs =
	       (ACPI_EC_Type *)(ACPI_EC_0_BASE + num * ACPI_EC_STRUCT_SIZE);
...
void acpi_write_str(enum acpi_ec_interface num, u8_t byte)
{
	ACPI_EC_Type *acpi_ec_regs =
	       (ACPI_EC_Type *)(ACPI_EC_0_BASE + num * ACPI_EC_STRUCT_SIZE);
...
{
	for (u16_t i = 0; i < HOST_TIMEOUT; i++) {
