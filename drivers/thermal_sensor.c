...
static u8_t adc_ch_bits;

s16_t adc_temp_val[ADC_CH_TOTAL];

static u8_t num_of_adc_ch;


static s16_t conv_adc_temp(u16_t AdcRawVal)
{
	/* Lpat table index for 0 temperature */
#define LPAT_TBL_0C_INDEX	80

	/* Lpat table Maximum negative temperature */
#define MAX_NEGATIVE_TEMP	-40

	/* Lpat table Maximum positive temperature Index */
#define MAX_POSITIVE_TEMP_INDX	330
 ...
 */
	...
	static const struct ThrmLPATTbl LpatTbl[] = {
	...
	/* 0 temperature */
	if (AdcRawVal == LpatTbl[LPAT_TBL_0C_INDEX].RawValue) {
		Temperature = 0;
	}
	/* -ve temperature overflow */
	else if (AdcRawVal >= LpatTbl[0].RawValue) {
		Temperature = LpatTbl[0].Temperature;
	}
	/* +ve temperature overflow */
	else if (AdcRawVal <= LpatTbl[MAX_POSITIVE_TEMP_INDX].RawValue) {
		Temperature = LpatTbl[MAX_POSITIVE_TEMP_INDX].Temperature;
	}

	else if (AdcRawVal > LpatTbl[LPAT_TBL_0C_INDEX].RawValue) {
		/* -ve temperature */
		for (Cnt = 0; Cnt <= LPAT_TBL_0C_INDEX; Cnt++) {
			if (AdcRawVal >= LpatTbl[Cnt].RawValue) {
				Temperature = LpatTbl[Cnt].Temperature;
				break;
			}
		}
	} else {
		/* +ve temperature */
		for (Cnt = LPAT_TBL_0C_INDEX; Cnt <= MAX_POSITIVE_TEMP_INDX;
				Cnt++) {
			if (AdcRawVal >= LpatTbl[Cnt].RawValue) {
				Temperature = LpatTbl[Cnt].Temperature;
				break;
...
int thermal_sensors_init(u8_t adc_channel_bits)
{
	int ret = 0;
	...
	if (adc_channel_bits == 0) {
		LOG_INF("NO adc sensor to enable");
		return -ENOTSUP;
	}
...
void thermal_sensors_update(void)
{
	...
	int ret;
	s16_t adc_raw_val[num_of_adc_ch];
	u8_t ch, ch_cnt = 0;
	...
	for (ch = ADC_CH_00; ch < ADC_CH_TOTAL; ch++) {
		...
		LOG_INF("ADC Ch %d : %d", ch, adc_temp_val[ch]);
	}
}
