...
sub top_of_kernel_tree {
	...
	my @tree_check = (
		...
		"west.yml",
		"app", "boards", "linker", "misc", "scripts"
	);
...
#trailing whitespace
		...
		if ($rawline =~ /\@(brief|param|retval).*[^\.]$/ ) {
			my $extracted_string = get_quoted_string($line, $rawline);
				WARN("documentation",
				     "Missing period at end of documentation line " . $extracted_string . "\n" . $herecurr);
		}
		...
		if ($realfile =~ /Kconfig/ &&
			...
			if ($is_start && $is_end && $length < $min_conf_desc_length) {
				WARN("CONFIG_DESCRIPTION",
				     "please write a paragraph that describes the config symbol fully ($min_conf_desc_length lines at least)\n" . $herecurr);
			}
...
# check for DT compatible documentation
		if (defined $root &&
			...
			my $dt_path = $ENV{ZEPHYR_BASE} . "/dts/bindings/";
